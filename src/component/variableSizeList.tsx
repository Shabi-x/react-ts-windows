...
interface VariableSizeProps extends Props {
...
const getItemMetadata = (
  ...
  if (index > lastMeasuredIndex) {
    ...
    for (let i = lastMeasuredIndex + 1; i <= index; i++) {
      let size = (itemSize as (index: number) => number)(i);
...
const VariableSizeList = createListComponent({
  ...
  getOffsetForIndexAndAlignment: (
    ...
    const maxOffset = Math.max(
      0,
      Math.min(estimatedTotalSize - size!, size!)
    );
  ...
  initInstanceProps(props: Props, instance: any): InstanceProps {
    const { estimatedItemSize } = props as VariableSizeProps;
